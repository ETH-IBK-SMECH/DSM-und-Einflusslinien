classdef TestRandbedingungenKondensieren < matlab.unittest.TestCase
    methods (Test)
        ...
        function spc_on_inactive_dof_is_ignored(tc)
            ...
            M = tests.util.ModelBuilder.minimal(2,3);
            S = tests.util.ModelBuilder.oneBarHoriz(M); S.sRelease = 1;
            M.Stab=S; M.Info.nStaebe=1;
            [DOF,nDOF,~]=dofNummerieren(M);

            K = speye(nDOF); F = zeros(nDOF,1);
        ...
        function matches_condensation_with_known_ui(tc)
            ...
            M.Stab=S; M.Info.nStaebe=1;
            [DOF,nDOF,~]=dofNummerieren(M);
            K = gallery('poisson',2); K = kron(K,eye(3)); K = K(1:nDOF,1:nDOF); K=K+K.'+speye(nDOF);
            F = (1:nDOF).';
            ...
            isKnown=false(1,nDOF); U=zeros(nDOF,1);
            localIdx=(1-1)*3+2; g=DOF(localIdx); isKnown(g)=true; U(g)=0.5;

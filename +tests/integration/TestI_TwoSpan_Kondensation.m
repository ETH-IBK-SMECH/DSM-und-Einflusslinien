...
%   - Rotation invariance with condensation present
    ...
    methods (TestClassSetup)
        function build_two_span_base(tc)
            ...
            E = tc.C.E; A = tc.C.A; I = tc.C.I; L = tc.C.L;

            % 3 nodes: (0,0) (L,0) (2L,0)
            M = tests.util.ModelBuilder.minimal(3, 3);
            M.Knoten(1).x = 0;    M.Knoten(1).y = 0;
            M.Knoten(2).x = L;    M.Knoten(2).y = 0;
            M.Knoten(3).x = 2*L;  M.Knoten(3).y = 0;

            % Bars 1:(1-2), 2:(2-3)
            S1 = tests.util.ModelBuilder.oneBarHoriz(M, 1, 2, E, A, I); S1.L=L;
            S2 = tests.util.ModelBuilder.oneBarHoriz(M, 2, 3, E, A, I); S2.L=L;
            M.Stab = [S1; S2];
            M.Info.nStaebe    = 2;
            M.Info.nKnotenDOF = 3;
    ...
    methods (Test)
        ...
        function condense_middle_node_all_components_matches_full_external(tc)
            ...
            Mc = tc.M2;
            Mc.StabLast = Mf.StabLast; Mc.Info.nStabLasten = 1;
        ...
        function condense_middle_node_partial_components_matches_full_external(tc)
            ...
            q = -8e3; nkd = tc.M2.Info.nKnotenDOF;
            ...
            Mc = tc.M2; Mc.StabLast = Mf.StabLast; Mc.Info.nStabLasten = Mf.Info.nStabLasten;
        ...
        function rueckrechnung_rebuilds_internal_equals_full_free_solution(tc)
            ...
            Mc = tc.M2; Mc.StabLast = Mf.StabLast; Mc.Info.nStabLasten = 1;
            Mc.Kondensation = struct('Knoten', 2, 'KomponentenMaske', logical([1 1 1]));
            outC = tests.util.it_run_main(Mc);
...
function [gextF, gextC] = externalDOFs(outF, outC, nodeList)
    ...
    gextF = []; gextC = [];
    for n = nodeList(:).'
        gf = outF.DOF((n-1)*nkd + (1:nkd)); gf = gf(gf>0);
        gc = outC.DOF((n-1)*nkd + (1:nkd)); gc = gc(gc>0);
...
function M = rotateAll(M, theta)
    x0 = M.Knoten(1).x; y0 = M.Knoten(1).y;
    c = cos(theta); s = sin(theta);
    for k = 1:numel(M.Knoten)
        x = M.Knoten(k).x - x0; y = M.Knoten(k).y - y0;
